...
for pair in 1Y:DGS1 2Y:DGS2 3Y:DGS3 5Y:DGS5 7Y:DGS7 10Y:DGS10; do
  sid="${pair#*:}"
  curl -fsS -m 60 "https://fred.stlouisfed.org/graph/fredgraph.csv?id=${sid}&cosd=${COSD}&coed=${COED}" -o "$TMP/${sid}.csv"
...
if len(rows) < 2000:
    raise SystemExit(f"only {len(rows)} rows parsed (expected ~2700+); refusing to overwrite bundled data")
out = {
...
js = "window.TNOTE_DATA = " + json.dumps(out, separators=(",",":")) + ";\n"
open("tnote-rates.js.tmp","w").write(js)      # write atomically so a failure never leaves a partial file
os.replace("tnote-rates.js.tmp", "tnote-rates.js")
